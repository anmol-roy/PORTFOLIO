...
const SOCIALS = [
  { icon: Github,   label: "GitHub",   sub: "@anmol-roy",       href: "https://github.com/anmol-roy",           color: "#94a3b8", hoverBg: "rgba(148,163,184," },
...
function CopyButton({ text, color }: { text: string; color: string }) {
  ...
  return (
    <motion.button onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
      ...
      style={{ color: copied ? color : "rgba(255,255,255,0.22)", background: copied ? `${color}14` : "transparent" }}
      title="Copy">
...
function InputField({ label, name, type="text", placeholder, value, onChange, required, rows }: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label className="font-mono uppercase tracking-[0.18em]"
        style={{ fontSize: "11.05px", color: focused ? "rgba(147,197,253,0.65)" : "rgba(255,255,255,0.28)" }}>
        {label}{required && <span className="ml-1" style={{ color:"#60a5fa" }}>*</span>}
      </label>
      <div className="relative">
        <motion.div className="absolute inset-0 rounded-xl pointer-events-none"
          animate={{ boxShadow: focused
            ? "0 0 0 1.5px rgba(147,197,253,0.3), 0 0 20px rgba(147,197,253,0.07)"
            : "0 0 0 1px rgba(255,255,255,0.07)" }}
          transition={{ duration: 0.2 }} style={{ borderRadius:"0.75rem" }}/>
        <Tag name={name} type={type} placeholder={placeholder} value={value} rows={rows}
          onChange={(e: React.ChangeEvent<HTMLInputElement|HTMLTextAreaElement>) => onChange(e.target.value)}
          onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
          required={required}
          className="font-dm w-full rounded-xl bg-white/[0.03] px-4 py-3 placeholder-white/20 outline-none resize-none"
          style={{ fontSize:"14.95px", color:"rgba(255,255,255,0.82)", border:"none" }}/>
      </div>
    </div>
  );
}
    ...
    <section id="contact" className="relative px-6 sm:px-10 lg:px-16 py-14 sm:py-20" style={{ background:"transparent" }}>
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <motion.div {...fadeUp(0)} className="mb-6 flex items-center gap-4">
          <span className="font-mono tracking-[0.3em] uppercase" style={{ fontSize:"12.35px", color:"rgba(196,181,253,0.5)" }}>07 —</span>
          <span className="font-syne tracking-[0.2em] uppercase" style={{ fontSize:"12.35px", color:"rgba(255,255,255,0.28)" }}>Contact</span>
        ...
        <motion.div {...fadeUp(0.05)} className="mb-10">
          ...
          <p className="font-dm mt-3 max-w-md leading-relaxed"
            style={{ fontSize:"15.6px", color:"rgba(255,255,255,0.35)" }}>
        ...
        <div className="grid lg:grid-cols-[1fr_350px] gap-4 items-start">
            ...
            <AnimatePresence mode="wait">
              {status === "sent" ? (
                <motion.div key="success"
                  ...
                  <div>
                    <p className="font-syne font-bold mb-1" style={{ fontSize:"18.85px", color:"rgba(255,255,255,0.82)" }}>
                      Message sent!
                    </p>
                    <p className="font-dm" style={{ fontSize:"14.95px", color:"rgba(255,255,255,0.35)" }}>
                ...
                <motion.form key="form" ref={formRef} onSubmit={handleSubmit}
                  ...
                  <motion.button type="submit" disabled={status==="sending"}
                    ...
                    className="font-dm mt-1 flex items-center justify-center gap-2 rounded-xl py-3.5 font-semibold text-white transition-all duration-300"
                    style={{ fontSize:"15.6px", background:"linear-gradient(135deg,rgba(147,197,253,0.15),rgba(196,181,253,0.12))", border:"1px solid rgba(147,197,253,0.22)" }}
                    onMouseEnter={(e)=>{ (e.currentTarget as HTMLButtonElement).style.background="linear-gradient(135deg,rgba(147,197,253,0.25),rgba(196,181,253,0.2))"; }}
                    onMouseLeave={(e)=>{ (e.currentTarget as HTMLButtonElement).style.background="linear-gradient(135deg,rgba(147,197,253,0.15),rgba(196,181,253,0.12))"; }}>
                    <AnimatePresence mode="wait">
                      {status === "sending" ? (
                        <motion.div key="spin" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
                          className="flex items-center gap-2.5">
                          <motion.div className="h-4 w-4 rounded-full border-2 border-white/20 border-t-blue-300"
          ...
          <div className="flex flex-col gap-3">
            ...
            <motion.div {...fadeUp(0.12)}
              ...
              style={{ border:"1px solid rgba(255,255,255,0.07)", background:"rgba(255,255,255,0.03)" }}>
              <p className="font-mono mb-4 tracking-[0.28em] uppercase" style={{ fontSize:"10.4px", color:"rgba(255,255,255,0.2)" }}>Info</p>
              ...
              <div className="flex flex-col gap-3">
                {INFO_ITEMS.map(({ icon:Icon, color, text, copyable }) => (
                  <div key={text} className="flex items-center justify-between group">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="font-dm" style={{ fontSize:"13.65px", color:"rgba(255,255,255,0.52)" }}>{text}</span>
                    </div>
                ...
                {/* Availability */}
                <div className="mt-1 flex items-center gap-2.5 pt-3" style={{ borderTop:"1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <span className="font-dm" style={{ fontSize:"13.65px", color:"rgba(52,211,153,0.72)" }}>
            ...
            <motion.div {...fadeUp(0.16)}
              className="rounded-2xl backdrop-blur-sm p-5"
              style={{ border:"1px solid rgba(255,255,255,0.07)", background:"rgba(255,255,255,0.03)" }}>
              <p className="font-mono mb-4 tracking-[0.28em] uppercase" style={{ fontSize:"10.4px", color:"rgba(255,255,255,0.2)" }}>Elsewhere</p>
                ...
                {SOCIALS.map(({ icon:Icon, label, sub, href, color, hoverBg }, i) => {
                  ...
                  return (
                    <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer"
                      ...
                      style={{ border:`1px solid ${hov ? color+"28" : "transparent"}`, background: hov ? `${hoverBg}0.06)` : "transparent" }}
                      ...
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl transition-all duration-200"
                        style={{ background: hov ? `${color}18` : "rgba(255,255,255,0.04)", border:`1px solid ${hov ? color+"30" : "rgba(255,255,255,0.06)"}`, boxShadow: hov ? `0 0 12px ${color}30` : "none" }}>
                        <Icon className="h-3.5 w-3.5 transition-colors duration-200" style={{ color: hov ? color : "rgba(255,255,255,0.28)" }}/>
                      </div>

                      <div className="flex flex-col min-w-0">
                        <span className="font-dm font-medium leading-none mb-0.5 transition-colors duration-200"
                          style={{ fontSize:"14.3px", color: hov ? color : "rgba(255,255,255,0.58)" }}>{label}</span>
                        <span className="font-mono truncate transition-colors duration-200"
                          style={{ fontSize:"11.05px", color: hov ? `${color}70` : "rgba(255,255,255,0.22)" }}>{sub}</span>
                      </div>
