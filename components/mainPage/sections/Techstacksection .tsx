...
const STACK: { category: string; items: Tech[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Next.js",    icon: "devicon-nextjs-plain",                        color: "#e2e8f0" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Linux",      icon: "devicon-linux-plain",                         color: "#fde68a" },
      { name: "AWS",        icon: "devicon-amazonwebservices-plain-wordmark",     color: "#fb923c" },
      { name: "GitHub",     icon: "devicon-github-original",                     color: "#cbd5e1" },
    ],
  },
];
    ...
    >
      ...
      <div
        ...
        onMouseEnter={(e) => {
          ...
          el.style.background = `${tech.color}1e`;
        ...
        onMouseLeave={(e) => {
          ...
          el.style.border = `1px solid ${tech.color}30`;
          el.style.boxShadow = `inset 0 1px 0 ${tech.color}18`;
          const icon = el.querySelector("i") as HTMLElement;
          if (icon) icon.style.color = `${tech.color}80`;
          const label = el.parentElement?.querySelector("span") as HTMLElement;
          if (label) label.style.color = "rgba(255,255,255,0.28)";
        }}
      >
        <i
    ...
    >
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <motion.div {...fadeUp(0)} className="mb-6 flex items-center gap-4">
          <span className="text-[11px] tracking-[0.3em] text-white/25 uppercase">
            05 — Tech Stack
          </span>
          <span className="h-px w-[60px] bg-white/[0.08]" />
        </motion.div>

        <motion.p {...fadeUp(0.05)} className="mb-8 text-sm text-white/25">
        ...
        <div className="flex flex-col divide-y divide-white/[0.04]">
          {STACK.map((group, gi) => (
            ...
            >
              {/* Category label */}
              <span className="w-24 shrink-0 text-[10px] tracking-[0.2em] text-white/20 uppercase">
                {group.category}
              </span>
